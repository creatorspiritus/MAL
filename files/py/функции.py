...
def Перечень_аэродромов(аэродром='UNWW', радиус=1000):
    ...
    for _ in набор.index:
        дистанция = Расстояние(аэродром, набор.loc[_]['Широта КТА'], набор.loc[_]['Долгота КТА'])
        if дистанция <= радиус:
            запись = {
                "ICAO": _,
                "Широта": набор.loc[_]['Широта КТА'],
                "Долгота": набор.loc[_]['Долгота КТА'],
                "Превышение": набор.loc[_]['Превышение'],
                "Расстояние": дистанция
            }
            возврат.append(запись)
    del d
