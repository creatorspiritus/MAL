...
def Дельта_Т(тип="Ил-114-300", расстояние=309, скороподъёмность=):
    возврат = None
    
    x = [0,3,4,5,6,7,8,9,10,11]
    y = [0,0.165,0.17,0.177,0.19,0.22,0.25,0.3,0.355,0.435]
    ...
    время_подъёма_минимум = 
    горизонтальный_полёт_минимальный = подъём + горизонт + спуск
